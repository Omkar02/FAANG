...
def CodeTimeLogging(Flag, filename, Tag, Difficult):
    ...
    if filename not in prevEntry:
        ...
        prevEntry[filename]['Duration'] = divmod(duration_in_s, 60)[0]
        print(f'Completed in = {divmod(duration_in_s, 60)[0]} mins')
...
def saveEntry(taskInfo):
    ...
    with open('loggingInfo.csv', 'w') as w:
        w.write('Name,StartDate,EndDate,StartTime,EndTime,Duration,Tag,Difficult\n')
    ...
    for keys, val in taskInfo.items():
        ...
        with open('loggingInfo.csv', 'a') as a:
            a.write(f'{keys},{StartDate},{EndDate},{StartTime},{EndTime},{Duration},{Tag},{Difficult}\n')
...
def getPreviousEntryDict():
    ...
    for allInfo in information:
        Name, StartDate, EndDate, StartTime, EndTime, Duration, Tag, Difficult = allInfo.split(',')
        ...
        currDict['Tag'] = Tag.strip()
        currDict['Difficult'] = Difficult.strip()
...
# CodeTimeLogging(Flag='F', filename='gg', Tag='Dynamic-Programming')
